                ...
                <table class="table table-bordered">
                  ...
                  <tbody>
                    @foreach($categories as $key => $category)
                      <tr>
                        <td>{{$key+1}}</td>
                        <td><img src="/images/categories/{{$category->image}}" width="100" height="80"></td>
                        <td>{{$category->name}}</td>
                        <td width="10%" class="p-0 pt-3">
                            <div class="actions text-center">
                              <span>
                                ...
                                <a href="{{route('categories.destroy',$category->id)}}" id="deleteClick{{$category->id}}" style="color: red; visibility: hidden;"></a>
                                <form action="{{ url('/categories', ['id' => $category->id]) }}" method="post" id="delete{{$category->id}}">
                                  @method('delete')
                                  @csrf
                                </form>
                              </span>
